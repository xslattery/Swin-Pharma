...
namespace web_api.Controllers
{
    ...
    {
        // Generateing the weekly forecast, single item:
        [HttpGet]
        [Route("Individual/Week/")]
        [ProducesResponseType(200, Type = typeof(string))]
        public IActionResult GetWeekSingle
            ([FromQuery(Name = "todayDate")] string today, [FromQuery(Name = "date")] string date)
        {
        ...
        // Generating the weekly forcast, group of items:
        [HttpGet]
        [Route("Group/Week/")]
        [ProducesResponseType(200, Type = typeof(string))]
        public IActionResult GetWeekGroup([FromQuery(Name = "todayDate")] string today, [FromQuery(Name = "date")] string date)
        {
            System.Diagnostics.Debug.WriteLine("########## FORECAST GetWeekGroup GET Inventory");
            Console.WriteLine("########## FORECAST GetWeekGroup GET Inventory");

            DateTime startDate;
            DateTime todayDate;
            if (DateTime.TryParse(date, new CultureInfo("en-AU"), System.Globalization.DateTimeStyles.AssumeLocal, out startDate) && DateTime.TryParse(today, new CultureInfo("en-AU"), System.Globalization.DateTimeStyles.AssumeLocal, out todayDate))
            {
                // Initiallising Required Controller
                if (!InventoryController.itemTableLoadedFromFile)
                {
                    InventoryController c = new InventoryController();
                }
        ...
        // Generateing the monthly forecast, single item:
        [HttpGet]
